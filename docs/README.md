...
**Attribution (required site-wide).** Waytide is a project **by [Eventide](https://eventide-project.org)**, and its purpose is to build Eventide's credibility — so the Eventide connection must be visible on every page, not buried. The built site should carry "Waytide — a project by Eventide" with a link back to Eventide in the header or footer, plus an About/Why page noting it grew out of Eventide's design practice. Waytide keeps its own brand and identity; the attribution is what lets the credibility flow back to Eventide.
