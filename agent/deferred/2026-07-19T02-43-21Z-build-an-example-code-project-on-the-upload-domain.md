...
- Its own `agent/` tree exercising the foundation / DBE / docs conventions **in the act of building** — rules installed via subtree, and real `design/`, `plans/`, `loops/` (and possibly `experiments/`) artifacts produced as the `Upload` solution is originated, so the conventions are shown *in use*, not just described.
